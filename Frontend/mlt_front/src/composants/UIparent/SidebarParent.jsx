...
import { LayoutDashboard, Baby, UserCircle, LogOut, ChevronRight, MessageSquare } from 'lucide-react';
...
const SidebarParent = () => {
    ...
    // Gestion de la déconnexion via le service d'authentification
    const handleLogout = () => {
        LogoutAction();
        navigate('/login');
    };
    ...
    const menuItems = [
    { name: 'Aperçu',      icon: <LayoutDashboard size={24} />, path: '/parent/dashboard' },
    { name: 'Mes Enfants', icon: <Baby size={24} />,            path: '/parent/enfants' },
    { name: 'Messagerie',  icon: <MessageSquare size={24} />,   path: '/parent/messagerie' },
    { name: 'Mon Profil',  icon: <UserCircle size={24} />,      path: '/parent/profil' },
    ];
            ...
            <div className="h-32 flex items-center px-8">
                <div className="flex items-center gap-4 cursor-pointer group" onClick={() => navigate('/parent/dashboard')}>
                    ...
                    <div className="hidden lg:block">
                        ...
                        <p className="text-[10px] font-bold opacity-40 uppercase tracking-[0.3em] mt-2">Espace Parent</p>
                    </div>
                </div>
            </div>
                        ...
                        return (
                            <NavLink
                                ...
                                className={`flex items-center justify-between px-8 py-4 transition-all duration-300 group border-l-4 ${
                                    isActive ? 'bg-base-200 border-primary text-primary' : 'bg-transparent border-transparent text-base-content/60 hover:text-primary hover:bg-base-200/50'
                                }`}
                            >
                                <div className="flex items-center gap-4">
                                    <span className={`${isActive ? 'text-primary' : 'opacity-40 group-hover:text-primary'}`}>{item.icon}</span>
                                    <span className={`hidden lg:block text-base tracking-tight ${isActive ? 'font-black' : 'font-bold'}`}>{item.name}</span>
                                </div>
